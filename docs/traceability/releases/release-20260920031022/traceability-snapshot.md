...
| LOAD_CHECKOUT | Commerce HTTP API | Safe authenticated checkout load with sandbox provider boundary | Fixture creation reached hosted DB schema mismatch (`terms_version_snapshot`); no financial request sent | BLOCKED_EXTERNAL |
...
| LOAD_ACTIVATION | Licensing challenge/action API | Authenticated license/device fixture; no challenge request was sent | `pnpm test:load -- --scenario=activation` returned `BLOCKED_EXTERNAL_TEST_FIXTURE` | BLOCKED_EXTERNAL |
...
| SEP3 | Durable chain command, worker, indexer and finality services | Hosted staging DB schema currently rejects order creation before command creation; pipeline harness remains ready | V2 runtime config and worker/indexer reconcile entrypoints | BLOCKED_EXTERNAL |
