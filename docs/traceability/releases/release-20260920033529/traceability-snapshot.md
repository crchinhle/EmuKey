...
|---|---|---|---|---|
| WEB_E2E | `frontend/e2e/phase6-license-flow.spec.ts`, Playwright config, `scripts/run-web-e2e.mjs` | Full external purchase/provider/admin/support/knowledge coverage and credentials | Public catalog/BC-05 Playwright smoke passed; customer flow blocked by test identity | BLOCKED_EXTERNAL |
| ANDROID_E2E | `mobile/e2e/phase6-license-flow.yaml`, Maestro readiness checker | Device/emulator execution and device receipt | `pnpm e2e:check`, Maestro evidence | BLOCKED_EXTERNAL |
...
| LOAD_ACTIVATION | Licensing challenge/action API | Canonical local Hardhat fixture requires a stable activation/device projection before concurrent load; no unsafe load is claimed | Existing golden flow rerun exposed a local finality projection mismatch | BLOCKED_EXTERNAL |
...
| SEP3 | Durable chain command, worker, indexer and finality services | Real staging customer/payment path still needs hosted deployment | Local target-schema API/worker harness, V2 Sepolia runtime config, reconcile entrypoint | BLOCKED_EXTERNAL |
| SEP4 | Web/Mobile surfaces and SePay adapter | Public HTTPS callback and full customer-to-entitlement flow | External E2E evidence | BLOCKED_EXTERNAL |
...
| STAGING_SCHEMA | Target PostgreSQL schema and seed | Hosted DB migration remains external; disposable target DB is verified | `staging-db-verification.json`, initialization/seed twice/verify | LOCAL_VERIFIED |
...
`PHASE8_COMPLETE_PRODUCTION_BLOCKED_EXTERNAL`

Repository-fixable Phase 8 gates in this matrix are closed. Remaining blockers require
staging infrastructure, a migrated target database, public HTTPS callback/DNS, external
test identity/device/recipient, or credential rotation authority.
