...
const outDir = path.join(root, 'visual-regression', 'runtime-before');
...
const selectors = {
  ...
  header: '.site-header',
  nav: '.public-nav a',
  title: 'main h1, .workspace-page-header h1, main h2',
  body: 'body',
};
...
for (const vp of viewports) {
  ...
  for (const route of routes) {
    let record = { route, viewport: vp.name, ok: false };
    try {
      ...
      const fonts = await page.evaluate(() => ({
        greatVibes: globalThis.document.fonts.check('34px "Great Vibes"'),
        greatVibes32: globalThis.document.fonts.check('340px "Great Vibes"'),
        plex: globalThis.document.fonts.check('16px "IBM Plex Sans"'),
      }));
      const data = await page.evaluate(({ selectors, customInputs }) => {
        const grab = (sel) => {
          ...
          return { fontFamily: s.fontFamily, fontSize: parseFloat(s.fontSize) || null, lineHeight: s.lineHeight, fontWeight: s.fontWeight, display: s.display, padding: s.padding, width: Math.round(r.width), height: Math.round(r.height) };
        };
        const out = {};
      ...
      record = { route, viewport: vp.name, ok: true, fonts, metrics: data };
    } catch (error) {
...
await writeFile(path.join(outDir, 'runtime-before.json'), JSON.stringify({ generatedAt: new Date().toISOString(), base, results }, null, 2));
